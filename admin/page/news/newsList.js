layui.config({
    ...
    var isclass = GetQueryString("class");

    //执行加载页面数据

    var arr = ["教程软件", "线报活动", "唯美头像", "其他文章"];

    var classname = arr[isclass];
    ...
    function Load(PageNumber) {
        ...
        $.post("../../Handle.ashx", { "parameter": "CheckList", "NEWSclass": PageNumber }, function (data) {
        ...
        function renderDate(data, curr) {
            ...
            if (currData.length != 0) {
                for (var i = 0; i < currData.length; i++) {
                    ...
                    //}
                    dataHtml += '<td>' + currData[i].lrNumber + '</td>'
